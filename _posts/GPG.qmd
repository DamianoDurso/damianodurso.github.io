...
## Linear regression
A very simple approach to calculate the AGPG is to run a linear regression including the different variables we expect to pay in addition to those that may have an impact on pay but only due to discrimination. When we apply this approach using our data
...
paste0('AGPG based on linear regression =' ,round((exp(coef(mod)['Gender_fac'])-1)*100,3))

```

## Oaxaca Blinder
A limitation of the linear regression is that we assume that the relationship between 
...
#summary(twofold)
paste0('AGPG based on Oaxaca-Blinder regression =' ,round((twofold$gaps$pct_gap*-1)*100,3))
```
...
oaxaca_gap_bootstrap <- function(data, indices) {
  ...
  pct_gap <- oaxaca_res$twofold$gaps$pct_gap * -1 * 100
...
bootstrap_results <- boot(data = df_clean, statistic = oaxaca_gap_bootstrap, R = 10)

# Calculate 95% CI for the gender pay gap
gap_ci <- boot.ci(bootstrap_results, type = "perc")

# Display results
cat("Bootstrap Results for Overall Gender Pay Gap:\n")
cat("Estimated Pay Gap (Mean):", mean(bootstrap_results$t), "%\n")
cat("95% CI for Gender Pay Gap:", gap_ci$percent[4:5], "%\n")
...
## Matching 
A procedure that allows us to account for group comparability is that of matching. 

Matching is a known approach to address common support issues. Unlike simple linear regression and Oaxaca, matching creates pairs or groups of individuals with similar characteristics across treatment groups (e.g., male and female) and then compares them directly. Here, we use full matching to ensure each observation finds the closest comparable counterpart.
